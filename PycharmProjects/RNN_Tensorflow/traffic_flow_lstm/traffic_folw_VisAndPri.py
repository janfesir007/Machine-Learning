...
class StockLSTM(object):  # StockLSTM(): LSTM模型训练封装成类
    ...
    def __init__(self, is_training, config):  # 类似于C++的“构造函数”
        ...
        lstm_cell = rnn_cell.BasicLSTMCell(size, forget_bias=1.0)  # 封装好的LSTM单元
        ...
        outputs, states = rnn.rnn(cell, inputs, initial_state=self._initial_state)  # outputs:[num_steps,batch_size,size],num_steps个[batch_size,size]矩阵：即每个step的输出结果
...
def main(data_file, is_diff):
    ...
    def run_epoch(session, m, data, eval_op, verbose=False):
        ...
        return outputs, costs_RMSE / (iters if iters > 0 else 1), costs_MAPE / (iters if iters > 0 else 1)  # 返回计算结果（预测值）和均方根误差RMSE和相对误差MAPE
    ...
    with tf.Graph().as_default(), tf.Session() as session:
        ...
        train_data, valid_data, test_data = Divide_data(data_file)  # 分割经过差分处理后的数据集:都以一维数组(narray)形式存放
        valid_test_data = np.array(list(test_data) + list(valid_data))  # 将验证集那部分数据也作为测试集加入
    ...
    with np.load(c.save_file) as file_load:  # 加载原始数据
        orig_train_data, orig_valid_data, orig_test_data = [file_load['arr_%d' % i] for i in
                                                            range(len(file_load.files))]
    ...
    for i, data in enumerate(pridict_test_onebyone):  # enumerate:既能输出索引又能输出值
        if is_diff == True:  # "差分"后的还原处理,还原公式：orig_x = pridict + orig_last_X
...
if __name__ == '__main__':
    ...
    start_time = time.time()
    # 画出对比图：传统plt
    show_pric_data = main(c.save_file, is_diff=False)[-200:]  # 输入数据,得到预测结果,列表类型
    show_lenth = len(show_pric_data)
    show_orig_data = listing_data[-show_lenth:]['Flow (Veh/Hour)']

    fig = plt.figure(figsize=(12, 8))  # 画布
    ax1 = fig.add_subplot(111)  # 坐标系
    plt.plot(np.arange(show_lenth), show_orig_data, marker="*", ms=6.0, linewidth=0.5, color="b")  # plt.plot(x,y);ms=markersize
    plt.plot(np.arange(show_lenth), show_pric_data, marker="o", ms=6.0, linewidth=0.5, color="r")
    xticks_dates = []  # x轴的刻度(ticks)显示日期
    ...
    # 设置x轴刻度上的标签:plt.xticks() 第1个参数：需要显示标签的横坐标的位置(int),每个5个位置显示标签,共显示20处;
    #  第2个参数：需要显示标签的20个位置所对应的具体内容;第3个参数rotation: x轴刻度标签旋转多少度.
    plt.xticks(np.arange(20) * (show_lenth/20), xticks_dates, rotation=45)
    plt.show()
    end_time = time.time()
