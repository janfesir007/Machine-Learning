...
def lstm_model(num_units, rnn_layers, dense_layers=None, learning_rate=0.1, optimizer='Adagrad'):
    ...
    def lstm_cells(layers):  # 封装lstm单元,返回多个BasicLSTMCell
    ...
    def dnn_layers(input_layers, layers):  # 返回最后一隐藏层的输出结果
    ...
    def _lstm_model(X, y):  # 返回“预测值/损失值/优化操作”
...
regressor = tflearn.TensorFlowEstimator(model_fn=lstm_model(TIMESTEPS, RNN_LAYERS, DENSE_LAYERS))
...
regressor.fit(X['train'], y['train'],  # 模型拟合
              monitors=[validation_monitor],
              batch_size=BATCH_SIZE,
              steps=TRAINING_STEPS)
predicted = regressor.predict(X['test'])  # 模型预测
